...
class VideoSource(models.Model):
    ...
    PROCESSING_STATUS_CHOICES = [
        ('pending', 'Pending All Processing'), ('metadata_fetched', 'Metadata Fetched'),
        ('transcript_processing', 'Transcript Processing'), ('transcript_processed', 'Transcript Processed'),
        ('transcript_unavailable', 'Transcript Unavailable'), ('transcript_failed', 'Transcript Processing Failed'),
        ('visual_downloading', 'Visual: Downloading Video'), ('download_failed', 'Visual: Download Failed'),
        ('visual_processing', 'Visual: Processing Frames'), ('visual_processed', 'Visual: Processing Complete'),
        ('visual_skipped', 'Visual: Processing Skipped/Not Applicable'), ('visual_failed', 'Visual Processing Failed'),
        ('analysis_complete', 'All Analysis Complete'), ('processing_failed_general', 'Processing Failed (General)'),
    ]
    processing_status = models.CharField(max_length=30, choices=PROCESSING_STATUS_CHOICES, default='pending', db_index=True, help_text="Overall content processing status for this source.")
    ...
    meta_visual_processing_status = models.CharField(max_length=20, choices=VISUAL_DETAIL_STATUS_CHOICES, default='pending', null=True, blank=True, db_index=True, help_text="Detailed status of visual feature extraction.")
    ...
    class Meta:
        ...
        indexes = [models.Index(fields=['last_scraped_at'], name='api_videosource_scraped_idx'), models.Index(fields=['processing_status'], name='api_vs_proc_status_idx'), models.Index(fields=['meta_visual_processing_status'], name='api_vs_meta_vis_stat_idx'),]
        verbose_name = "Video Source Instance"; verbose_name_plural = "Video Source Instances"
    def __str__(self): return f"{self.platform_name} ({self.platform_video_id or 'N/A'}) - {self.video.title[:50] if self.video else 'N/A'}"

class Transcript(models.Model):
    id = models.BigAutoField(primary_key=True)
    video_source = models.OneToOneField(VideoSource, related_name='transcript_data', on_delete=models.CASCADE, db_index=True)
    ...
    PROCESSING_STATUS_CHOICES = [('pending', 'Pending'), ('processed', 'Processed'), ('failed', 'Failed'), ('not_available', 'Not Available'),]
    ...
    class Meta: verbose_name = "Video Transcript"; verbose_name_plural = "Video Transcripts"; indexes = [models.Index(fields=['processing_status'], name='api_transcript_proc_stat_idx')]
    def __str__(self): return f"Transcript for VSID {self.video_source_id} ({self.language_code})"

class ExtractedKeyword(models.Model):
    id = models.BigAutoField(primary_key=True)
    video_source = models.ForeignKey(VideoSource, related_name='extracted_keywords', on_delete=models.CASCADE, db_index=True)
    ...
    created_at = models.DateTimeField(auto_now_add=True)
    class Meta: unique_together = ('video_source', 'keyword_text', 'source_field'); verbose_name = "Extracted Keyword"; verbose_name_plural = "Extracted Keywords"; indexes = [models.Index(fields=['keyword_text'], name='api_keyword_text_idx')]
    def __str__(self): return self.keyword_text

class VideoTopic(models.Model):
    id = models.BigAutoField(primary_key=True)
    video_source = models.ForeignKey(VideoSource, related_name='video_topics', on_delete=models.CASCADE, db_index=True)
    ...
    created_at = models.DateTimeField(auto_now_add=True)
    class Meta: unique_together = ('video_source', 'topic_name', 'modeling_method'); verbose_name = "Video Topic"; verbose_name_plural = "Video Topics"
    def __str__(self): return self.topic_name
...
class VideoFrameFeature(models.Model):
    ...
    timestamp_ms = models.PositiveIntegerField(help_text="Timestamp of frame in milliseconds from video start.")
    ...
    class Meta:
        ...
        indexes = [models.Index(fields=['video_source', 'timestamp_ms'], name='api_frame_time_idx'), models.Index(fields=['feature_type', 'hash_value'], name='api_frame_hash_idx'),]
        verbose_name = "Video Frame Feature"; verbose_name_plural = "Video Frame Features"
    def __str__(self): return f"{self.feature_type} for VSID {self.video_source_id} at {self.timestamp_ms}ms"

class UserProfile(models.Model):
    user = models.OneToOneField(User, on_delete=models.CASCADE, related_name='profile', primary_key=True)
    subscription_plan_choices = [('free_trial', 'Free Trial'), ('papri_pro_monthly', 'Papri Pro Monthly'), ('papri_pro_yearly', 'Papri Pro Yearly'), ('cancelled', 'Cancelled'),]
    ...
    created_at = models.DateTimeField(auto_now_add=True); updated_at = models.DateTimeField(auto_now=True)
    def __str__(self): return f"Profile for {self.user.username} (Plan: {self.get_subscription_plan_display()})"
...
class SearchTask(models.Model):
    ...
    STATUS_CHOICES = [
        ('pending', 'Pending Submission'), ('queued', 'Queued for Processing'), ('processing', 'Processing Query'),
        ('processing_sources', 'Processing Sources & Content'), ('aggregating', 'Aggregating Results'),
        ('completed', 'Completed Successfully'), ('partial_results', 'Completed with Partial Results'), ('failed', 'Failed'),
        ('failed_query_understanding', 'Failed: Query Understanding'), ('failed_source_fetch', 'Failed: Source Fetching'),
        ('failed_content_analysis', 'Failed: Content Analysis'), ('failed_aggregation', 'Failed: Result Aggregation'),
        ('failed_timeout', 'Failed: Task Timeout'), ('cancelled', 'Cancelled by User'),
    ]
    status = models.CharField(max_length=30, choices=STATUS_CHOICES, default='pending', db_index=True)
    error_message = models.TextField(null=True, blank=True)
    celery_task_id = models.CharField(max_length=255, null=True, blank=True, db_index=True)
    created_at = models.DateTimeField(auto_now_add=True); updated_at = models.DateTimeField(auto_now=True)
    class Meta: ordering = ['-created_at']; verbose_name = "User Search Task"; verbose_name_plural = "User Search Tasks"
    def __str__(self): return f"SearchTask {self.id} ({self.get_status_display()})"

class SignupCode(models.Model):
    id = models.BigAutoField(primary_key=True)
    email = models.EmailField(unique=True, help_text="Email address the code was sent to.")
    code = models.CharField(max_length=20, unique=True, db_index=True)
    plan_name = models.CharField(max_length=100, default="Papri Pro Monthly")
    ...
    created_at = models.DateTimeField(auto_now_add=True); updated_at = models.DateTimeField(auto_now=True)
    class Meta: verbose_name = "User Signup Code"; verbose_name_plural = "User Signup Codes"
    def __str__(self): return f"Code {self.code} for {self.email} (Used: {self.is_used})"
    @property
    def is_expired(self):
...
class VideoEditProject(models.Model):
    ...
    created_at = models.DateTimeField(auto_now_add=True); updated_at = models.DateTimeField(auto_now=True)
    class Meta: ordering = ['-created_at']; verbose_name = "AI Video Edit Project"; verbose_name_plural = "AI Video Edit Projects"
    def __str__(self): return f"Edit Project: '{self.project_name}' by {self.user.username}"

class EditTask(models.Model):
    STATUS_CHOICES = [
        ('pending', 'Pending Submission'), ('queued', 'Queued for Processing'), ('processing', 'Processing Video'),
        ('downloading_video', 'Downloading Video'), ('interpreting_prompt', 'Interpreting Prompt'),
        ('editing_in_progress', 'Editing in Progress'), ('uploading_output', 'Uploading Output'),
        ('completed', 'Completed Successfully'), ('failed', 'Failed (Generic)'),
        ('failed_download', 'Failed: Video Download'), ('failed_prompt_interpretation', 'Failed: Prompt Interpretation'),
        ('failed_editing', 'Failed: Video Editing Process'), ('failed_output_storage', 'Failed: Output Storage'),
    ]
    id = models.UUIDField(primary_key=True, default=uuid.uuid4, editable=False)
    ...
    error_message = models.TextField(null=True, blank=True)
    created_at = models.DateTimeField(auto_now_add=True); updated_at = models.DateTimeField(auto_now=True)
    class Meta: ordering = ['-created_at']; verbose_name = "AI Video Edit Task"; verbose_name_plural = "AI Video Edit Tasks"
    def __str__(self): return f"Edit Task {self.id} for Project '{self.project.project_name}' ({self.get_status_display()})"
    def get_result_url(self) -> Optional[str]:
        if not self.result_media_path: return None
        if self.result_media_path.startswith(('http://', 'https://')): return self.result_media_path
        if hasattr(settings, 'MEDIA_URL') and settings.MEDIA_URL:
            media_url_base = str(settings.MEDIA_URL).rstrip('/'); relative_path = self.result_media_path.lstrip('/')
